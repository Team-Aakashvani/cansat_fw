...
int TelemetryEncoder::encode(const TelemetryFrame& f, char* out, size_t out_len) const noexcept {
    ...
    int n = snprintf(out, out_len,
        ...
        (double)f.accel_x_mps2, (double)f.accel_y_mps2, (double)f.gyro_z_dps, (unsigned)f.software_state,
...
TelemetryFrame TelemetryEncoder::make_frame(
    ...
    if (gnss.valid) {
        f.latitude_deg = (float)gnss.lat_deg; f.longitude_deg = (float)gnss.lon_deg;
        f.gnss_alt_msl_m = (float)gnss.alt_msl_m; f.satellites = gnss.satellites;
        strncpy(f.gnss_time_str, gnss.time_str, sizeof(f.gnss_time_str) - 1);
    } else { strncpy(f.gnss_time_str, "00:00:00", sizeof(f.gnss_time_str)); }
    if (imu.valid) {
        f.accel_x_mps2 = (float)imu.acc_x; f.accel_y_mps2 = (float)imu.acc_y;
